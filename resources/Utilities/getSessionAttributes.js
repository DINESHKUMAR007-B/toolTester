...
exports.getSessionAttributes = function getSessionAttributes(attributes, intentRequest, type, messages, callback) {
   ...
   try {
      ...
      appSession.appSessionObj.dialogActionType = type;
      let loggingData = logData.UpdateLogData(intentRequest, type, messages);
      logger.info(loggingData);
      appSession.audioStartTimeOut = appSession.appSessionObj.dialogActionType == "StartIntent" || appSession.appSessionObj.dialogActionType == "Close" ?
         process.const.Lex_IntentTimeout : appSession.appSessionObj.dialogActionType == "ElicitSlot" ? process.const.Lex_Audio_Start_Timeout_Input :
            appSession.appSessionObj.dialogActionType == "ElicitIntent" ? process.const.Lex_Audio_Start_Timeout_Menu : appSession.audioStartTimeOut;
      appSession.audioEndTimeOut = appSession.audioStartTimeOut == process.const.Lex_IntentTimeout ?
         process.const.Lex_IntentTimeout : appSession.appSessionObj.dialogActionType == "ElicitSlot" ? process.const.Lex_Audio_End_Timeout_Input :
            appSession.appSessionObj.dialogActionType == "ElicitIntent" ? process.const.Lex_Audio_End_Timeout_Menu : appSession.audioEndTimeOut;
      appSession.dtmfEndTimeOut = appSession.audioStartTimeOut == process.const.Lex_IntentTimeout ?
         process.const.Lex_IntentTimeout : appSession.appSessionObj.dialogActionType == "ElicitSlot" ? process.const.Lex_DTMF_End_Timeout :
            appSession.appSessionObj.dialogActionType == "ElicitIntent" ? "600" : appSession.dtmfEndTimeOut;
      attributes[process.const.STR_Interrupt + process.const.STR_Asterisk + process.const.STR_Colon + process.const.STR_Asterisk] =
         appSession.bargeIn == undefined ? process.const.STR_True : appSession.bargeIn;
      ...
      attributes.outputData = ivaHelper.OutputData(appSession, cxiSession);
      attributes[process.const.STR_Audio_Start + process.const.STR_Asterisk + process.const.STR_Colon + process.const.STR_Asterisk] = appSession.audioStartTimeOut == undefined ? "5000" : appSession.audioStartTimeOut;
      attributes[process.const.STR_Dtmf_End + process.const.STR_Asterisk + process.const.STR_Colon + process.const.STR_Asterisk] = appSession.dtmfEndTimeOut == undefined ? "600" : appSession.dtmfEndTimeOut;
